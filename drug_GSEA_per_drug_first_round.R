#DEAP package aims to find enriched substructures, targets, target class, pathways for the predictions from LINCS
#currently, it aims to find enriched targets. The targets are predicted from SEA.
#the method is adopted from GSEA

#per drug, across multiple cell lines

#input:
#drugs = c("sirolimus") #, "wortmannin", "sunitinib" , "tamoxifen", "strophanthidin", "sorafenib", "selamectin", "sirolimus","pyrvinium-pamoate","oligomycin-a","obatoclax","gefitinib")
#drug = "vorinostat"
drugs = c("FCCP", "mebendazole")
...
visualize_target_enrichment <- function(target, target_name, report, target_cmpd_rank){
  ...
  pdf(paste("target_enrich/", drug, "_", target,"_", cell_line, ".pdf", sep="")) 
    ...
    plot(x=c(1:length(gsea_o$RES)), y = gsea_o$RES, type = "l" , xlab = "", 
         ylab = "score", col = "red", main = paste(target_name, "enrichment"))
...
for(drug in drugs){
#you can specify one drug or multiple drugs
#find all drugs available in this cell line
files = list.files("drug_sim", pattern = drug)
cell_lines = sapply(files, function(drug_name){
  unlist(strsplit(drug_name, "_"))[4]
})
for (cell_line in cell_lines){
#input drug list
pred = read.csv(paste("drug_sim/", drug, "_sim_drugs_", cell_line, "_cells.csv", sep=""))

#if the input drug has a small number of drugs tested under the same condition. Skip this drug.
if (nrow(pred)<1000){next}

#suppose the input drug is most correlated to itself
pred = rbind(pred, data.frame(X=100000, pert_iname=drug, cor=1))
pred$pert_iname = tolower(pred$pert_iname)

#load drug targets
load("lincs_cmpd_info_mesh_target_sea.RData") #
...
#can change sea_targets to any other target resources, e.g., chembl
pred_targets = subset(pred_terms, !is.na(sea_targets) & sea_targets != "")
pred_targets$targets = pred_targets$sea_targets
...
write.csv(report, paste("drug_targets/", drug, "_sea_", cell_line, ".csv", sep=""))

#visualize the top 5 targets
report_top_hits = subset(report, NES > 0)
top_hits = head(report_top_hits[order(report_top_hits$`FDR q-val`),  "GS"], 5)
for (target in top_hits){
  visualize_target_enrichment(  target, target, report,target_cmpd_rank)
}
